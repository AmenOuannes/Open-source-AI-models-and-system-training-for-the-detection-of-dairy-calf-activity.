...
def transform_X_y(data):
    ...
    for i in range(len(data_np) - 50):
        row = [[j[4], j[7],j[8], j[9]] for j in data_np[i:i+40]]
        X.append(row)
        annotations = data_np[i+40:i+50]  # The label is the data point after the window
...
def generate_train(calf_values, calf_labels):
    ...
    X = X.reshape((instances , 40, 4)) #reshape from (5, n, 40, 4) to (5*n, 40, 4) to keep a linear structure
    y = np.array(y)
...
def leave_one_calf_out(calf_values, calf_labels, id_, features=4):
    ...
    X_train = X_train.reshape((instances, 40, features))  #reshape
    ...
    X_id = X_id.reshape((X_id.shape[0], 40, features))
...
def confusion_lstm(model, X, y):
    ...
    #plot with seaborn
    sns.heatmap(cm_normalized_recall, annot=True, xticklabels=["down", "drink", "up"],
